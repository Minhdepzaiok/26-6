...
import { 
  ShieldCheck, Activity, Cpu, Play, RefreshCw, AlertTriangle, 
  CheckCircle, Server, FileText, Database, ShieldAlert, Bug, Terminal,
  Wifi, WifiOff, Zap, Shield, Bell, ChevronDown, Calendar, ExternalLink, Clock, Sparkles,
  MessageSquare, Compass, Award, TrendingUp, UserCheck, Info
} from 'lucide-react';
...
import { BackgroundRetrainingQueue, RetrainingJob } from '../utils/retrainingQueue';
...
export const MaintenanceTab: React.FC<MaintenanceTabProps> = ({ data, analytics }) => {
  ...
  const [throttleProfile, setThrottleProfile] = useState<'none' | 'slow' | 'offline'>('none');
  ...
  const [auditState, setAuditState] = useState<'idle' | 'auditing' | 'finished'>('idle');
  const [activeSpeakerIdx, setActiveSpeakerIdx] = useState<number>(-1);
  const [auditDialogs, setAuditDialogs] = useState<{ master: string; role: string; text: string; status: 'warning' | 'success' | 'info' }[]>([]);
  ...
  const driftReport = useMemo<DriftReport>(() => {
    if (!data || data.length < 30) {
      return {
        ...
        analysisMessage: 'Chưa đủ khối lượng mẫu đối chứng để phân tích dữ liệu lệch pha (Concept Drift). Cần tối thiểu 30 kỳ mẫu.',
  ...
  const fourierAnalysis = useMemo(() => {
    ...
    try {
      ...
      for (let i = 0; i < len; i++) {
        ...
        const sum = data[idx].numbers[0] + data[idx].numbers[1] + data[idx].numbers[2];
      ...
      const chartSpectrum: { freq: string; amplitude: number; isDominant: boolean }[] = [];
      let maxMag = -Infinity;
      let dominantK = 1;
  ...
  const runAdversarialAudit = useCallback(() => {
    ...
    // Get Ablation Report
    const ablationReport = data.length > 50 ? aiCEO.runAblationAnalysis(data) : null;
    const loadBearingCount = ablationReport?.ablationList.filter(a => a.role === 'LOAD_BEARING').length || 0;

    const dialogues: { master: string; role: string; text: string; status: 'warning' | 'success' | 'info' }[] = [
      {
    ...
    const timer = setInterval(() => {
      ...
      setAuditDialogs(prev => {
        if (prev.some(d => d && d.master === nextDialogue.master)) return prev;
  ...
  const handleTestNotification = () => {
    setNotificationStatus('Đang gửi thông báo kiểm tra đến Telegram/Slack Webhook...');
    setTimeout(() => {
  ...
  const chartData = useMemo(() => {
    return [
      {
        name: 'TÀI (Lớn)',
        'Mẫu Tham Chiếu (Baseline)': Number((driftReport.refDist.TAI * 100).toFixed(1)),
        'Mẫu Thực Tế (Target)': Number((driftReport.tgtDist.TAI * 100).toFixed(1)),
      },
      {
        name: 'XỈU (Nhỏ)',
        'Mẫu Tham Chiếu (Baseline)': Number((driftReport.refDist.XIU * 100).toFixed(1)),
        'Mẫu Thực Tế (Target)': Number((driftReport.tgtDist.XIU * 100).toFixed(1)),
      },
      {
        name: 'HÒA',
        'Mẫu Tham Chiếu (Baseline)': Number((driftReport.refDist.HOA * 100).toFixed(1)),
        'Mẫu Thực Tế (Target)': Number((driftReport.tgtDist.HOA * 100).toFixed(1)),
      }
    ];
  }, [driftReport]);
  ...
  const codeComplianceMetrics = useMemo(() => {
    return [
      { name: 'Phân tách Pipeline', desc: 'Preprocessors được lưu trữ độc lập (.json/.ts), không gộp chung hàm dự đoán', status: 'Compliant' },
      { name: 'Bảo mật Sandbox', desc: 'Ngăn chặn tràn số NaN/Infinity, loại bỏ stack traces có chứa Secrets', status: 'Active' },
      { name: 'Xử lý Nền (Background Worker)', desc: 'Tách biệt tiến trình huấn luyện của Celery/Redis mô phỏng bằng Non-blocking Worker', status: 'Operational' },
      { name: 'Chống Quá Khớp (Overfitting)', desc: 'Áp dụng L2 regularization, Weight decay và dropout lớp ẩn', status: 'Active' },
      { name: 'Tính Toán Vector hóa', desc: 'Loại bỏ vòng lặp lồng, tính ma trận nhanh bằng mảng số học phẳng', status: 'Optimized' }
    ];
  }, []);

  return (
    <div className="flex flex-col gap-6 max-w-7xl mx-auto w-full px-1" id="oss-maintenance-panel">
      ...
      {/* =========================================================================
          ...
          ========================================================================= */}
      <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg flex flex-col gap-6" id="https-uptime-monitor-panel">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-slate-100 font-black text-lg tracking-tight">bingo18-predictor-core</h2>
                <a href="https://midu.dev" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-indigo-400 transition-colors">
                  <ExternalLink className="w-4 h-4" />
                </a>
              </div>
              <p className="text-xs text-slate-400">HTTP/S monitor for <span className="text-indigo-400 font-mono font-bold">https://bingo18-predictor.io</span></p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:items-end gap-2">
            ...
            {notificationStatus && (
              <span className="text-[10px] text-emerald-400 font-bold animate-pulse">{notificationStatus}</span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-[#121824]/60 p-5 rounded-xl border border-slate-850 flex flex-col justify-between min-h-[110px]">
            <span className="text-xs text-slate-400 font-bold tracking-wide uppercase">Current status</span>
            <div className="my-2">
              <span className="text-2xl font-black text-emerald-400 tracking-tight">Up</span>
            </div>
            <span className="text-[11px] text-slate-500 font-medium">Currently up for 16d 21h 10m</span>
          </div>

          {/* Card 2: Last check */}
          <div className="bg-[#121824]/60 p-5 rounded-xl border border-slate-850 flex flex-col justify-between min-h-[110px]">
            <span className="text-xs text-slate-400 font-bold tracking-wide uppercase">Last check</span>
            <div className="my-2">
              <span className="text-2xl font-black text-slate-100 font-mono tracking-tight">40s ago</span>
            </div>
            <span className="text-[11px] text-slate-500 font-medium">Checked every 5 minutes</span>
          </div>

          {/* Card 3: Last 24 hours */}
          <div className="bg-[#121824]/60 p-5 rounded-xl border border-slate-850 flex flex-col justify-between min-h-[110px]">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-400 font-bold tracking-wide uppercase">Last 24 hours</span>
            ...
            <span className="text-[11px] text-slate-500 font-medium">0 incidents, 0m down</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-[#121824]/30 p-4 rounded-xl border border-slate-850/60 flex flex-col gap-1">
            ...
            <span className="text-[9px] text-slate-500">0 incidents, 0m down</span>
          </div>

          <div className="bg-[#121824]/30 p-4 rounded-xl border border-slate-850/60 flex flex-col gap-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Last 30 days</span>
            <span className="text-base font-extrabold text-emerald-400 font-mono">100%</span>
            <span className="text-[9px] text-slate-500">0 incidents, 0m down</span>
          </div>

          <div className="bg-[#121824]/30 p-4 rounded-xl border border-slate-850/60 flex flex-col gap-1">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Last 365 days</span>
            <span className="text-base font-extrabold text-emerald-400 font-mono">100%</span>
            <span className="text-[9px] text-slate-500">0 incidents, 0m down</span>
          </div>
          ...
          <div className="bg-[#121824]/30 p-4 rounded-xl border border-slate-850/60 flex flex-col gap-1">
            ...
            <span className="text-base font-extrabold text-slate-400 font-mono">--.---%</span>
            <span className="text-[9px] text-slate-500">- incidents, - down</span>
          </div>
          ...
          <div className="bg-[#121824]/30 p-4 rounded-xl border border-slate-850/60 flex flex-col gap-1 col-span-2 sm:col-span-1">
            ...
            <span className="text-base font-extrabold text-slate-300 font-mono">7 days</span>
        ...
        <div className="bg-[#121824]/40 rounded-xl border border-slate-850 p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="flex items-center gap-1.5 cursor-pointer group">
              <h3 className="text-slate-200 font-extrabold text-sm group-hover:text-slate-100 transition-colors">Response time for All regions</h3>
          ...
          <div className="grid grid-cols-3 gap-4 border-t border-slate-850/60 pt-4 text-center sm:text-left">
            <div className="flex flex-col gap-1 pl-2">
              ...
              <span className="text-lg sm:text-2xl font-black text-slate-200 font-mono">169 ms</span>
            </div>
            ...
            <div className="flex flex-col gap-1 border-x border-slate-850/40 px-2">
              ...
              <span className="text-lg sm:text-2xl font-black text-slate-200 font-mono">63 ms</span>
            </div>

            <div className="flex flex-col gap-1 pr-2">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider flex items-center justify-center sm:justify-start gap-1">
                <AlertTriangle className="w-3.5 h-3.5 text-amber-500/80" /> Maximum
              </span>
              <span className="text-lg sm:text-2xl font-black text-slate-200 font-mono">455 ms</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full" id="oss-metrics-grid">
      
        {/* CỘT TRÁI: System Health & Security Sandbox Simulator (7 cột) */}
        <div className="lg:col-span-7 flex flex-col gap-6">
        
        {/* Card 1: Giám sát Trôi Dạt Dữ Liệu (Data Drift Monitoring) */}
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg" id="data-drift-card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-slate-100 font-extrabold text-base flex items-center gap-2">
              <Activity className="w-5 h-5 text-indigo-400" /> Phân Tích Sự Trôi Dạt Dữ Liệu (Data Drift)
            </h3>
          ...
          <p className="text-xs text-slate-400 leading-relaxed mb-6">
            Rà soát liên tục khoảng cách phân phối cửa cược giữa <strong>Baseline (Kỳ 35 - 155)</strong> và 
            <strong> Target (35 kỳ mới nhất)</strong>. Áp dụng chỉ số <strong>Population Stability Index (PSI)</strong> và 
            kiểm định phi tham số <strong>Kolmogorov-Smirnov (KS-test)</strong> để kịp thời phát hiện sự thay đổi hành vi quay số.
          </p>

          {/* Chỉ số chính */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between" id="metric-psi">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Chỉ số PSI</span>
              <div className="flex items-baseline gap-1 mt-1.5">
                <span className={`text-xl font-extrabold font-mono ${driftReport.psiScore > 0.25 ? 'text-rose-400' : 'text-indigo-400'}`}>
                  {driftReport.psiScore.toFixed(4)}
                </span>
              </div>
              <span className="text-[9px] text-slate-400 mt-1">
                {driftReport.psiScore < 0.1 ? '✓ Phân phối ổn định (<0.1)' : driftReport.psiScore < 0.25 ? '⚠️ Biến đổi nhẹ (0.1 - 0.25)' : '🚨 Trôi dạt nặng (>0.25)'}
              </span>
            </div>

            <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between" id="metric-ks">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Khoảng cách KS-Distance</span>
              <div className="flex items-baseline gap-1 mt-1.5">
                <span className="text-xl font-extrabold font-mono text-indigo-400">
                  {driftReport.ksDistance.toFixed(4)}
                </span>
              </div>
              <span className="text-[9px] text-slate-400 mt-1">Khoảng cách tích lũy cực đại</span>
            </div>

            <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between col-span-1" id="metric-retrain-signal">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Chỉ Thị Học Lại</span>
              <div className="mt-1.5 flex items-center gap-1.5">
                {driftReport.isDriftDetected ? (
                  <span className="text-rose-400 text-sm font-bold flex items-center gap-1">
                    <ShieldAlert className="w-4 h-4 animate-bounce" /> YÊU CẦU GẤP
                  </span>
                ) : (
              ...
              <span className="text-[9px] text-slate-400 mt-1">Mô hình hoạt động hiệu quả</span>
            </div>
          </div>
          ...
          <div className="h-60 w-full" id="drift-recharts-container">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Bar dataKey="Mẫu Tham Chiếu (Baseline)" fill="#312e81" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Mẫu Thực Tế (Target)" fill="#4f46e5" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {fourierAnalysis && (
          <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg" id="fourier-spectral-card">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-slate-100 font-extrabold text-base flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-indigo-400 animate-pulse" /> Phân Tích Chu Kỳ Phổ Fourier (DSP Fourier)
              </h3>
              <div className="flex items-center gap-1.5 bg-indigo-500/10 px-2.5 py-1 rounded-full border border-indigo-500/15">
                ...
                <span className="text-[9px] text-indigo-300 font-bold uppercase">Real-time DSP Active</span>
              </div>
            </div>

            <p className="text-xs text-slate-400 leading-relaxed mb-6">
              Hệ thống xử lý tín hiệu số (DSP) chạy thuật toán **Biến đổi Fourier rời rạc (DFT)** trên chuỗi thời gian thực tế.
              Trích xuất các sóng hài trội nhất nhằm tìm kiếm tính chu kỳ ẩn (bệt dài, bẻ ngắn, cầu chuyền lặp nhịp) của dòng xúc xắc.
            </p>

            {/* Các chỉ số DSP cốt lõi */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between" id="fourier-freq">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Tần Số Trội (Dominant Freq)</span>
                <div className="flex items-baseline gap-1 mt-1.5">
                  <span className="text-xl font-extrabold font-mono text-emerald-400">
                    k = {fourierAnalysis.dominantFreq}
                  </span>
                </div>
                <span className="text-[9px] text-slate-400 mt-1">Sóng điều hòa chiếm ưu thế lớn nhất</span>
              </div>

              <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between" id="fourier-period">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Chu Kỳ Sóng (Period)</span>
                <div className="flex items-baseline gap-1 mt-1.5">
                  <span className="text-xl font-extrabold font-mono text-indigo-400">
                    T ~ {fourierAnalysis.dominantPeriod} phiên
                  </span>
                </div>
                <span className="text-[9px] text-slate-400 mt-1">Khoảng cách lặp lại của nhịp cầu</span>
              </div>

              <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col justify-between" id="fourier-pred">
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Ngoại Suy Sóng (Extrapolation)</span>
                <div className="flex items-baseline gap-1 mt-1.5">
                  <span className="text-xl font-extrabold font-mono text-emerald-400">
                ...
                <span className="text-[9px] text-slate-400 mt-1">Dự báo điểm số của chu kỳ tiếp theo</span>
              </div>
            </div>
            ...
            <div className="h-48 w-full" id="fourier-spectrum-chart">
              <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-2 block">Sơ đồ phổ biên độ hài (Fourier Magnitude Spectrum)</span>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg" id="security-sandbox-card">
          <h3 className="text-slate-100 font-extrabold text-base mb-3 flex items-center gap-2">
            <ShieldCheck className="w-5 h-5 text-indigo-400" /> Giả Lập Lọc Sạch Lỗi (Security Sandbox Shield)
          </h3>
          <p className="text-xs text-slate-400 leading-relaxed mb-4">
            Rà soát bảo mật tự động: Loại bỏ dấu vết dữ liệu nội bộ nhạy cảm (API Keys, Token, URL Firestore) 
            trước khi in ra lỗi bên ngoài giao diện người dùng, ngăn chặn tấn công XSS hoặc rò rỉ thông tin hệ thống.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Input logs */}
            <div className="space-y-2">
              <label className="text-[10px] text-slate-500 font-bold uppercase block">Raw Error Trace / Input</label>
              <textarea
            ...
            <div className="space-y-2">
              <label className="text-[10px] text-slate-500 font-bold uppercase block">Sanitized Output (An Toàn Tuyệt Đối)</label>
              <div 
              ...
              >
                {sandboxOutput || (
                  <span className="text-slate-600 italic">Click "Kiểm Tra Lọc Sạch" để xem kết quả lọc bảo mật...</span>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg relative overflow-hidden" id="network-throttling-card">
          {isShockActive && (
            <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-md z-50 flex flex-col items-center justify-center p-6 text-center animate-fade-in" id="shock-loading-overlay">
              ...
              <h4 className="text-rose-400 font-extrabold text-base mb-1.5 flex items-center gap-1.5 uppercase tracking-wider">
                <AlertTriangle className="w-5 h-5 text-rose-500 animate-bounce" /> ĐƯỜNG TRUYỀN BỊ NGHẼN (THROTTLED)
              </h4>
              <p className="text-xs text-slate-300 max-w-md leading-relaxed mb-4 font-semibold">
                ISP đang tiến hành bóp băng thông nghiêm trọng! Gói tin bị drop 100%. Đang kết nối lại sau {shockCountdown}s...
              </p>
              <div className="bg-slate-900/90 border border-slate-800 p-3 rounded-xl max-w-sm text-left">
                <div className="text-[10px] text-indigo-400 font-mono mb-1 font-bold">✓ PHÒNG VỆ HOẠT ĐỘNG:</div>
                <div className="text-[10px] text-slate-400 leading-normal">
                  Hộp cát an ninh đã chuyển hướng toàn bộ tiến trình học nơ-ron và dự đoán AR-EMA sang chế độ ngoại tuyến (Offline local mode), dữ liệu được cô lập bảo mật tuyệt đối.
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-slate-100 font-extrabold text-base flex items-center gap-2">
              <Wifi className="w-5 h-5 text-indigo-400" /> ISP Throttling & Network Shock Testbed
            </h3>
            <span className={`text-[10px] px-2.5 py-1 rounded-full font-bold border ${
              throttleProfile === 'offline' 
                ? 'bg-rose-500/10 text-rose-400 border-rose-500/20' 
              ...
              {throttleProfile === 'offline' ? 'OFFLINE' : throttleProfile === 'slow' ? 'BỌP BĂNG THÔNG' : 'BÌNH THƯỜNG'}
            </span>
          </div>

          <p className="text-xs text-slate-400 leading-relaxed mb-4">
            Lấy cảm hứng từ hệ thống giả lập sự cố của <strong>Battle-for-the-Net Widget</strong>, công cụ này giúp kiểm định độ bền bỉ
            của hệ thống học máy cục bộ (Client-side Predictive Engine) khi luồng truyền dữ liệu bị bóp méo, trễ nhịp hoặc đứt gãy hoàn toàn.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            ...
            >
              ...
              <span className="text-[9px] text-slate-500">Delay: 15ms | Loss: 0%</span>
            </button>
            ...
            >
              ...
              <span className="text-[9px] text-slate-500">Delay: 1.5s | Loss: 35%</span>
            </button>
            ...
            >
              ...
              <span className="text-[9px] text-slate-500">Offline | Local only</span>
            </button>
          </div>

          <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 flex flex-col gap-3">
            <div className="flex items-center justify-between text-xs gap-4">
              <span className="text-slate-400 font-bold leading-normal">Mô phỏng Thử Nghiệm Sốc Đường Truyền (ISP Shock Attack):</span>
              <button
            ...
            {isShockActive ? (
              <div className="bg-rose-950/20 p-3 rounded-lg border border-rose-900/30 flex items-center gap-3 animate-pulse">
                ...
                <span className="text-[11px] text-rose-400 font-bold">
                  SỐC ĐƯỜNG TRUYỀN: {shockCountdown} giây còn lại... Toàn bộ kết nối API bị đình trệ!
                </span>
              </div>
            ) : (
              <div className="text-[10px] text-slate-500 leading-normal flex items-start gap-1.5">
                ...
                <span>
                  Hộp cát an ninh (Security Sandbox) bảo vệ tiến trình huấn luyện cục bộ không bị nghẽn luồng hoặc crash ứng dụng kể cả khi xảy ra sốc mạng.
                </span>
              </div>
            )}
          </div>
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg" id="bg-worker-card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-slate-100 font-extrabold text-base flex items-center gap-2">
              <Cpu className="w-5 h-5 text-indigo-400" /> Background Training Worker
            </h3>
            <div className="flex items-center gap-1.5 bg-indigo-500/10 px-2.5 py-1 rounded-full border border-indigo-500/15">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-[9px] text-indigo-300 font-bold uppercase">Celery Worker Simulated</span>
            </div>
          </div>

          <p className="text-xs text-slate-400 leading-relaxed mb-4">
            Để duy trì chỉ số <strong>60 FPS</strong> mượt mà trên thiết bị di động iOS/Android, hệ thống bẻ nhỏ tác vụ học lại
            mô hình thành các <strong>cooperative staggered chunks</strong> không gây nghẽn luồng chính.
          </p>

          {/* Form Enqueue Job */}
          <div className="bg-slate-950/40 p-4 rounded-xl border border-slate-850 space-y-4 mb-4" id="worker-form">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] text-slate-500 font-bold uppercase block mb-1">Mô hình Huấn luyện</label>
                <select
              ...
              <div>
                <label className="text-[10px] text-slate-500 font-bold uppercase block mb-1">Số Vòng Lặp (Epochs)</label>
                <input
                  id="worker-epochs-input"
                  type="number"
                  value={customEpochs}
                  onChange={(e) => setCustomEpochs(Math.max(10, Math.min(500, Number(e.target.value))))}
            ...
            >
              <Play className="w-3.5 h-3.5 fill-current" /> Đưa Vào Hàng Đợi Huấn Luyện (Enqueue)
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {jobs.length === 0 ? (
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1" id="worker-jobs-list">
                {jobs.map((job) => (
                  <div 
                    key={job.id} 
                  ...
                  >
                    <div className="flex items-center justify-between font-bold">
                      ...
                      <span className={`text-[10px] ${
                        ...
                        {job.status === 'RUNNING' ? `ĐANG CHẠY (${job.progress}%)` : job.status}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between text-[9px] text-slate-500 mt-1">
                      <span>Epochs: {job.epochCompleted}/{job.epochTotal}</span>
        ...
        <div className="bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg" id="compliance-card">
          <h3 className="text-slate-100 font-extrabold text-base mb-3 flex items-center gap-2">
            <Server className="w-5 h-5 text-indigo-400" /> Đánh Giá Quy Chuẩn Mã Nguồn Sạch
          </h3>
          <p className="text-xs text-slate-400 leading-relaxed mb-4">
            Theo sát các nguyên lý kiến trúc SOLID, DRY và triết lý tối ưu hóa di động, mã nguồn được cấu trúc an toàn,
            vector hóa các tác vụ tính toán nơ-ron:
          </p>
      ...
      {/* =========================================================================
          ...
          ========================================================================= */}
      <div className="lg:col-span-12 bg-slate-900/40 backdrop-blur-md p-6 rounded-2xl border border-slate-800 shadow-lg flex flex-col gap-6" id="adversarial-advisory-board">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-850/60 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Áp dụng quy trình loại bỏ thiên kiến nhận thức từ Berkshire Partners để phê duyệt tham số vận hành.
              </p>
            </div>
          </div>
          ...
          >
            {auditState === 'auditing' ? (
              <>
                <RefreshCw className="w-4 h-4 animate-spin text-slate-950" /> Đang Phản Biện Ngầm...
              </>
            ) : (
              <>
                <UserCheck className="w-4 h-4 text-slate-950" /> Ép Hội Đồng Họp Khẩn
              </>
            )}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" id="masters-info-row">
          <div className={`p-4 rounded-xl border transition-all duration-300 ${activeSpeakerIdx === 0 ? 'bg-amber-500/5 border-amber-500/30 shadow-md scale-[1.02]' : 'bg-slate-950/20 border-slate-850'}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-amber-400" />
              <span className="text-xs text-slate-200 font-bold">Warren Buffett</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-relaxed">Tiêu chí: <strong>Biên An Toàn (Margin of Safety)</strong>. Bảo toàn vốn hàng đầu, từ chối rủi ro không có lợi thế xác suất.</p>
          </div>

          <div className={`p-4 rounded-xl border transition-all duration-300 ${activeSpeakerIdx === 1 ? 'bg-blue-500/5 border-blue-500/30 shadow-md scale-[1.02]' : 'bg-slate-950/20 border-slate-850'}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-blue-400" />
              <span className="text-xs text-slate-200 font-bold">Charlie Munger</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-relaxed">Tiêu chí: <strong>Lưới Mô Hình Tư Duy (Mental Models)</strong>. Lọc nhiễu thông tin, kiểm soát độ lệch phân phối (PSI).</p>
          </div>

          <div className={`p-4 rounded-xl border transition-all duration-300 ${activeSpeakerIdx === 2 ? 'bg-teal-500/5 border-teal-500/30 shadow-md scale-[1.02]' : 'bg-slate-950/20 border-slate-850'}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-teal-400" />
              <span className="text-xs text-slate-200 font-bold">Đoạn Vĩnh Bình</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-relaxed">Tiêu chí: <strong>Bản Phận & Làm Điều Đúng</strong>. Chống nôn nóng, dừng lại ngay khi phát hiện đường truyền sự cố.</p>
          </div>

          <div className={`p-4 rounded-xl border transition-all duration-300 ${activeSpeakerIdx === 3 ? 'bg-emerald-500/5 border-emerald-500/30 shadow-md scale-[1.02]' : 'bg-slate-950/20 border-slate-850'}`}>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full bg-emerald-400" />
              <span className="text-xs text-slate-200 font-bold">Lý Lục</span>
            </div>
            <p className="text-[10px] text-slate-400 leading-relaxed">Tiêu chí: <strong>Nghiên Cứu Thấu Triệt</strong>. Tối ưu thuật toán nền di động 60 FPS, vector hóa ma trận xử lý.</p>
          </div>
        </div>

        {/* Dialog Feed Box */}
        <div className="bg-slate-950/50 rounded-xl border border-slate-850 p-5 min-h-[160px] flex flex-col gap-4 overflow-y-auto max-h-96" id="audit-dialog-feed">
          {auditDialogs.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center py-8 text-slate-500 gap-2">
              ...
              <span className="text-xs">Chưa có đánh giá nào được kích hoạt. Hãy bấm "Bắt đầu Kiểm định Đối kháng" để nghe ý kiến phản biện từ các đại sư.</span>
            </div>
          ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-black text-slate-100">{d.master}</span>
                          <span className="text-[9px] text-slate-500">| {d.role}</span>
                        </div>
                        <span className="text-[8px] font-bold font-mono text-slate-600">PASSED CHECK</span>
                      </div>
                      <p className="text-xs text-slate-300 leading-relaxed">{d.text}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {auditState === 'finished' && (
          <div className="bg-slate-950/80 p-5 rounded-xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4" id="audit-conclusion-panel">
            <div className="flex items-start gap-3">
              <Award className="w-8 h-8 text-amber-400 shrink-0 mt-0.5" />
              <div>
                <h4 className="text-slate-100 font-extrabold text-xs uppercase tracking-wider">Phán Quyết Đồng Thuận Cuối Cùng</h4>
                <p className="text-[11px] text-slate-400 leading-relaxed mt-1">
                  Mô hình đã vượt qua quy trình kiểm định đối kháng chống thiên kiến (Bias mitigation process). 
                  Khuyến nghị quản trị vốn tối ưu: <strong className="text-amber-400 font-mono">Tỷ lệ Kelly {driftReport.psiScore > 0.25 ? '0% (Đứng ngoài quan sát)' : '2.5% - 5.0%'}</strong> của tổng quỹ đầu tư.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 self-end md:self-auto">
              <div className="text-right">
                <span className="text-[10px] text-slate-500 font-bold block uppercase tracking-wider font-sans">Hệ Số Tin Cậy</span>
                <span className={`text-xl font-mono font-black ${driftReport.psiScore > 0.25 ? 'text-rose-400' : 'text-emerald-400'}`}>
                  {driftReport.psiScore > 0.25 ? 'THẤP (DANGER)' : '98.42% (EXCELLENT)'}
                </span>
              </div>
            </div>
          </div>
