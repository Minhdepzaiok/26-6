  ...
  public transform(draws: Draw[], lag = 5): {
    ...
    for (let i = 0; i < len; i++) {
      ...
      if (!d || !d.numbers || !Array.isArray(d.numbers)) {
        ...
        const s = d.numbers[0] + d.numbers[1] + d.numbers[2]; // Fixed size 3 for dice
    ...
    for (let i = 0; i < len; i++) {
      normalizedSums[i] = this.scaleMinMax(rawSumsBuffer[i], this.config.sumMin, this.config.sumMax);
    }
