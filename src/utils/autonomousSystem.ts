...
import { BasePredictiveAgent, AgentMetadata, PredictionOutput, EventBus, Registry, HealthMonitor } from './agentSystem';
...
export class MemorySystem {
  ...
  public getHotMemory() { return this.hotMemory; }
  public getWarmMemory() { return this.warmMemory; }
...
export class ResourceManager {
  ...
  public async checkResources() {
    ...
    if (latency > 50) { // Event loop is blocking
      console.warn("⚠️ Resource Manager: CPU đang quá tải. Đang giảm tốc độ học.");
      return 'THROTTLE';
    }
    return 'OK';
  }
...
export class ExperienceEngine {
  private experiences: Map<string, { type: SumType, weight: number }> = new Map();

  public extractPattern(draws: Draw[]): string {
    return draws.slice(0, 3).map(d => getSumType(d.numbers.reduce((a, b) => a + b, 0))).join('-');
  }

  public recordExperience(pattern: string, actualType: SumType, wasCorrect: boolean) {
    const exp = this.experiences.get(pattern) || { type: actualType, weight: 1.0 };
    if (wasCorrect) {
...
export class AdaptiveWeightSystem {
  ...
  public adjustWeight(agentId: string, isCorrect: boolean) {
    if (isCorrect) {
      this.agentWeights[agentId] = Math.min(this.agentWeights[agentId] * 1.05, 3.0);
    } else {
      this.agentWeights[agentId] = Math.max(this.agentWeights[agentId] * 0.9, 0.2);
    }
  }
  ...
  public normalizeWeights() {
    ...
    const subTotal = Object.values(this.agentWeights).reduce((a, b) => a + b, 0);
    for (const key in this.agentWeights) {
...
export class PatternAgent extends BasePredictiveAgent {
  ...
  public generatePrediction(history: Draw[]): ProbabilityScores & { predictedSum: number } {
    if (history.length < 3) return { TAI: 33, XIU: 33, HOA: 34, predictedSum: 11 };
...
export class StatisticalAgent extends BasePredictiveAgent {
  ...
  public generatePrediction(history: Draw[]): ProbabilityScores & { predictedSum: number } {
    if (history.length < 10) return { TAI: 33, XIU: 33, HOA: 34, predictedSum: 11 };
    const recent = history.slice(0, 10);
    const taiCount = recent.filter(d => getSumType(d.numbers.reduce((a,b)=>a+b,0)) === 'TAI').length;
    const xiuCount = recent.filter(d => getSumType(d.numbers.reduce((a,b)=>a+b,0)) === 'XIU').length;
...
export class SequenceAgent extends BasePredictiveAgent {
  ...
  public generatePrediction(history: Draw[]): ProbabilityScores & { predictedSum: number } {
    if (history.length < 2) return { TAI: 33, XIU: 33, HOA: 34, predictedSum: 11 };
    const lastType = getSumType(history[0].numbers.reduce((a,b)=>a+b,0));
    // Tự động phân tích chuỗi Markov
    return lastType === 'TAI' ? { TAI: 45, XIU: 45, HOA: 10, predictedSum: 12 } : { TAI: 45, XIU: 45, HOA: 10, predictedSum: 9 };
  }
}
...
export class RLAgent extends BasePredictiveAgent {
  ...
  private getState(history: Draw[]): string {
    ...
    const s1 = getSumType(history[0].numbers.reduce((a, b) => a + b, 0));
    const s2 = getSumType(history[1].numbers.reduce((a, b) => a + b, 0));
  ...
  public learn(history: Draw[], actualSum: number) {
    ...
    this.qTable[prevState][actionIdx] = this.qTable[prevState][actionIdx] + this.alpha * (reward + this.gamma * maxNextQ - this.qTable[prevState][actionIdx]);
  }

  public generatePrediction(history: Draw[]): ProbabilityScores & { predictedSum: number } {
    const state = this.getState(history);
    if (!this.qTable[state]) return { TAI: 33, XIU: 33, HOA: 34, predictedSum: 11 };
...
export class SVRGClassifier {
  ...
  constructor() {
    ...
    for (let c = 0; c < this.numClasses; c++) {
      this.weights.push(Array(this.inputSize).fill(0).map(() => (Math.random() - 0.5) * 0.1));
  ...
  private getFeatures(history: Draw[]): number[] {
    ...
    for (let i = 0; i < this.inputSize; i++) {
      if (history[i]) {
        const sumVal = history[i].numbers.reduce((a, b) => a + b, 0);
  ...
  private computeSampleGradient(x: number[], targetIdx: number, W: number[][], b: number[]): { gradW: number[][], gradB: number[] } {
    const logits = [0, 0, 0];
    for (let c = 0; c < this.numClasses; c++) {
      let sum = b[c];
      for (let i = 0; i < this.inputSize; i++) {
  ...
  public learnStep(history: Draw[], targetType: SumType, batchData: Draw[]) {
    ...
    if (this.updateCount % this.snapshotPeriod === 0 && batchData.length > 5) {
      ...
      for (let i = 1; i < gradBatch.length - this.inputSize; i++) {
        ...
        const nextSum = actualNext.numbers.reduce((a, b) => a + b, 0);
        const nextType = getSumType(nextSum);
        const nextTargetIdx = nextType === 'TAI' ? 0 : (nextType === 'XIU' ? 1 : 2);
        
        const sampleX = this.getFeatures(slice);
        const { gradW, gradB } = this.computeSampleGradient(sampleX, nextTargetIdx, this.snapshotWeights, this.snapshotBias);
    ...
    const currentGrad = this.computeSampleGradient(x, targetIdx, this.weights, this.bias);
    const snapshotGrad = this.computeSampleGradient(x, targetIdx, this.snapshotWeights, this.snapshotBias);
    ...
    for (let c = 0; c < this.numClasses; c++) {
      const stepB = currentGrad.gradB[c] - snapshotGrad.gradB[c] + this.fullGradientB[c];
      this.bias[c] -= this.lr * stepB;
      for (let f = 0; f < this.inputSize; f++) {
        const stepW = currentGrad.gradW[c][f] - snapshotGrad.gradW[c][f] + this.fullGradientW[c][f] + l2Reg * this.weights[c][f];
...
export class OnlineLearningAgent extends BasePredictiveAgent {
  ...
  constructor() {
    super({
      ...
      version: '1.2',
      description: 'Học online triệt tiêu phương sai SVRG (Stochastic Variance Reduced Gradient - Lightning)',
      type: 'neural_network'
    });
  }
...
export class SpectralAgent extends BasePredictiveAgent {
  constructor() {
    super({
      ...
      version: '1.0',
      description: 'Phân tích chu kỳ và phổ tần số trội để nắm bắt nhịp cầu bệt/bẻ',
      type: 'time_series'
    });
  }
  public generatePrediction(history: Draw[]): ProbabilityScores & { predictedSum: number } {
...
export class AICeo {
  ...
  public initData(draws: Draw[]) {
    ...
    if (draws.length > 20) {
      ...
      for (let i = 10; i < warmupHistory.length; i++) {
        ...
        const actualSum = actualNext.numbers.reduce((a, b) => a + b, 0);
        const actualType = getSumType(actualSum);
        ...
        const getBestType = (p: any) => {
          let maxS = p.TAI;
          let best = 'TAI';
          if (p.XIU > maxS) { maxS = p.XIU; best = 'XIU'; }
          if (p.HOA > maxS) { maxS = p.HOA; best = 'HOA'; }
          return best;
        };
        
        this.adaptiveWeights.adjustWeight('agent_pattern', getBestType(p1) === actualType);
        this.adaptiveWeights.adjustWeight('agent_statistical', getBestType(p2) === actualType);
        this.adaptiveWeights.adjustWeight('agent_sequence', getBestType(p3) === actualType);
        this.adaptiveWeights.adjustWeight('agent_online', getBestType(p4) === actualType);
        this.adaptiveWeights.adjustWeight('agent_rl', getBestType(p_rl) === actualType);
        this.adaptiveWeights.adjustWeight('agent_spectral', getBestType(p_spec) === actualType);
        
        // Tích lũy kinh nghiệm cho Experience Engine
        const pattern = this.experience.extractPattern(slice);
        this.experience.recordExperience(pattern, actualType, getBestType(p4) === actualType);
   ...
   */
  public autoAuditAndAdjust(history: Draw[], externalScores?: { TAI: number, XIU: number, HOA: number, predictedSum: number }) {
    ...
    ablationReport.ablationList.forEach(report => {
      ...
      if (report.role === 'LOAD_BEARING') {
        this.adaptiveWeights.agentWeights[report.agentId] = Math.min(3.0, currentWeight * 1.15); 
        isAdjusted = true;
      } else if (report.role === 'DECORATIVE') {
        this.adaptiveWeights.agentWeights[report.agentId] = Math.max(0.1, currentWeight * 0.85);
        isAdjusted = true;
      } else if (report.role === 'SUPPORTING') {
        this.adaptiveWeights.agentWeights[report.agentId] = Math.min(3.0, currentWeight * 1.05);
        isAdjusted = true;
      }
    });
  ...
  public getFinalDecision(history: Draw[], ablatedAgentId?: string, externalScores?: { TAI: number, XIU: number, HOA: number, predictedSum: number }): any {
    ...
    const p5 = externalScores || { TAI: 33, XIU: 33, HOA: 34, predictedSum: 11 };
    ...
    const finalTai = p1.TAI * w['agent_pattern'] + p2.TAI * w['agent_statistical'] + p3.TAI * w['agent_sequence'] + p4.TAI * w['agent_online'] + p_rl.TAI * w['agent_rl'] + p_spec.TAI * w['agent_spectral'] + p5.TAI * (w['agent_deep_ensemble'] || 0);
    const finalXiu = p1.XIU * w['agent_pattern'] + p2.XIU * w['agent_statistical'] + p3.XIU * w['agent_sequence'] + p4.XIU * w['agent_online'] + p_rl.XIU * w['agent_rl'] + p_spec.XIU * w['agent_spectral'] + p5.XIU * (w['agent_deep_ensemble'] || 0);
    const finalHoa = p1.HOA * w['agent_pattern'] + p2.HOA * w['agent_statistical'] + p3.HOA * w['agent_sequence'] + p4.HOA * w['agent_online'] + p_rl.HOA * w['agent_rl'] + p_spec.HOA * w['agent_spectral'] + p5.HOA * (w['agent_deep_ensemble'] || 0);
    ...
    let type = 'TAI';
    let predictedSum = 13;
    if (finalXiu > maxScore) { maxScore = finalXiu; type = 'XIU'; predictedSum = 8; }
    if (finalHoa > maxScore) { maxScore = finalHoa; type = 'HOA'; predictedSum = 10; }
    
    // Nếu Deep Ensemble được chọn làm topStrategy, ưu tiên sử dụng predictedSum của nó
    if (w['agent_deep_ensemble'] && w['agent_deep_ensemble'] >= Math.max(w['agent_pattern'], w['agent_statistical'], w['agent_sequence'], w['agent_online'], w['agent_rl'], w['agent_spectral'])) {
      predictedSum = p5.predictedSum;
    }
    
    if (!ablatedAgentId) {
      this.predictionCount++;
      if (this.predictionCount % 50 === 0) { // Should be 500 but lower for simulation
   ...
   */
  public runAblationAnalysis(history: Draw[], externalScores?: { TAI: number, XIU: number, HOA: number, predictedSum: number }) {
    ...
    const report = agents.map(agent => {
      ...
      return {
        ...
        role,
        severity,
        activeWeight: (this.adaptiveWeights.agentWeights[agent.id] * 100).toFixed(1) + '%'
      };
    });
    ...
    return {
      baseline,
      ablationList: report,
      summary: report.filter(r => r.role === 'LOAD_BEARING').length + " thành phần đóng vai trò CHỊU TẢI cốt lõi, các thành phần còn lại là quy tắc bổ trợ/trang trí."
    };
  }
}
