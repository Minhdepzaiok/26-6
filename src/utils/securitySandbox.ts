...
export class SecuritySandbox {
  ...
  public static sanitizeARParams(params?: ARParams): ARParams {
    ...
    const lag = typeof params.lag === 'number' && !isNaN(params.lag) && isFinite(params.lag)
      ? Math.max(2, Math.min(12, Math.round(params.lag)))
      : defaultParams.lag;

    const emaAlpha = typeof params.emaAlpha === 'number' && !isNaN(params.emaAlpha) && isFinite(params.emaAlpha)
      ? Math.max(0.01, Math.min(1.0, params.emaAlpha))
      : defaultParams.emaAlpha;

    const learningRate = typeof params.learningRate === 'number' && !isNaN(params.learningRate) && isFinite(params.learningRate)
    ...
    const epochs = typeof params.epochs === 'number' && !isNaN(params.epochs) && isFinite(params.epochs)
  ...
  public static sanitizeMLPParams(params?: MLPParams): MLPParams {
    ...
    const inputLags = typeof params.inputLags === 'number' && !isNaN(params.inputLags) && isFinite(params.inputLags)
    ...
    const hiddenNeurons = typeof params.hiddenNeurons === 'number' && !isNaN(params.hiddenNeurons) && isFinite(params.hiddenNeurons)
  ...
  public static sanitizeDraws(draws: Draw[]): Draw[] {
    ...
    return draws
      .map(draw => {
        ...
        const id = draw.id !== undefined && draw.id !== null
          ? String(draw.id).replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 50)
        ...
        numbers = numbers.map(n => {
          const num = typeof n === 'number' && !isNaN(n) && isFinite(n) ? Math.round(n) : 1;
