...
class GenericModelAgent extends BasePredictiveAgent {
  ...
  constructor(
    ...
    version: string,
    description: string,
    type: 'time_series' | 'neural_network' | 'bayesian' | 'ensemble' | 'heuristic' | 'reinforcement',
...
export const calculateAnalytics = (
  ...
  try {
    // TẠI SAO (Why): Áp dụng Sandbox an ninh rà soát & chuẩn hóa đầu vào chống tấn công vắt kiệt tài nguyên / DoS / Phá hỏng kiểu dữ liệu
    const sanitizedData = SecuritySandbox.sanitizeDraws(data);
    if (!sanitizedData || sanitizedData.length === 0) return null;

    const sanitizedAR = SecuritySandbox.sanitizeARParams(arParams);
    const sanitizedMLP = SecuritySandbox.sanitizeMLPParams(mlpParams);
    ...
    const topDrawsSignature = sanitizedData.slice(0, 5).map(d => `${d.id}-${d.numbers.join(',')}`).join('|');
    const cacheKey = `${sanitizedData.length}_${topDrawsSignature}_${selectedAlgo}_${JSON.stringify(sanitizedAR)}_${JSON.stringify(sanitizedMLP)}`;
  ...
  const chronological = [...sanitizedData].sort((a, b) => {
  ...
  initRegistryIfNeeded(chronological, sanitizedAR, sanitizedMLP);
  ...
  // Đóng gói (Serialize) & Gọi ra dùng đồng bộ ở cả khâu học và khâu dự đoán.
  const dataPipeline = new DataPipeline();
  
  // Kiểm tra nếu có cấu hình serialize trước đó trong localStorage (mô phỏng .pkl / .onnx)
  const savedPipelineStr = typeof window !== 'undefined' ? localStorage.getItem('ai_pipeline_config') : null;
  if (savedPipelineStr) {
    dataPipeline.deserialize(savedPipelineStr);
  } else {
    // FIX DATA LEAKAGE: Chỉ fit pipeline trên 80% tập dữ liệu cũ (Train Data), 
    // không để lộ dữ liệu tương lai (Test Data) vào bộ tính Min-Max/Scaler.
    const trainLen = Math.floor(chronological.length * 0.8);
    const trainData = chronological.slice(0, trainLen);
    dataPipeline.fit(trainData.length > 10 ? trainData : chronological);
    
    if (typeof window !== 'undefined') {
      localStorage.setItem('ai_pipeline_config', dataPipeline.serialize());
    }
  }
  
  // Transform data cho các mô hình
  const pipelineData = dataPipeline.transform(chronological);
  ...
  const mlpParamsObj = sanitizedMLP;
  ...
  aiCEO.initData(chronological);
  // Auto-run the 4-master council ablation to drive CEO weights dynamically
  aiCEO.autoAuditAndAdjust(chronological, {
    TAI: activeScoreTai,
    XIU: activeScoreXiu,
    HOA: activeScoreHoa,
    predictedSum: activePredictedSum
  });
  const aiCeoDecision = aiCEO.getFinalDecision(chronological, undefined, {
    TAI: activeScoreTai,
    XIU: activeScoreXiu,
    HOA: activeScoreHoa,
    predictedSum: activePredictedSum
  ...
  // Re-adjust using Autonomous System's output
  activePredictedType = aiCeoDecision.predictedType;
  activePredictedSum = aiCeoDecision.predictedSum;
  activeConfidence = Number(aiCeoDecision.confidence);
  activeScoreTai = aiCeoDecision.scores.TAI;
  activeScoreXiu = aiCeoDecision.scores.XIU;
  activeScoreHoa = aiCeoDecision.scores.HOA;
  activeDetectedPattern = `Tự trị AI (Autonomous Mobile CEO) - Chiến lược dẫn dắt: ${aiCeoDecision.topStrategy}`;
  ...
  const prediction: PredictionResult = {
    ...
    ablationReport: aiCEO.runAblationAnalysis(chronological)
  };
  ...
  const result: Analytics = {
    ...
    const secureError = SecuritySandbox.sanitizeErrorMessage(err);
    console.error("Critical error in calculateAnalytics (Sanitized):", secureError);
    return null;
  }
};
