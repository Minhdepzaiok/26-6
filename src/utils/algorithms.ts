...
export const runARForecast = (
  ...
  try {
    ...
    const emaValues = new Float64Array(n);
    emaValues[0] = sums[0];
    for (let k = 1; k < n; k++) {
    ...
    const d_dim = lag + 1;
    const x_est = new Float64Array(d_dim);
    ...
    const P_cov = Array(d_dim).fill(null).map((_, i) => {
      const r = new Float64Array(d_dim);
      r[i] = 10.0;
      return r;
    });
    ...
    for (let i = 0; i < numPairs; i++) {
      ...
      const H = new Float64Array(d_dim);
      for (let j = 0; j < lag; j++) {
      ...
      const PH = new Float64Array(d_dim);
      ...
      const K = new Float64Array(d_dim);
    ...
    const H_pred = new Float64Array(d_dim);
...
export const runMLPClassifier = (
  ...
  try {
    ...
    const W1 = Array(hiddenSize).fill(null).map(() => 
      new Float64Array(inputSize).map(() => (Math.random() * 2 - 1) * range1)
    );
    ...
    const W2 = Array(outputSize).fill(null).map(() => 
      new Float64Array(hiddenSize).map(() => (Math.random() * 2 - 1) * range2)
    );
    ...
    const mW1 = Array(hiddenSize).fill(null).map(() => new Float64Array(inputSize));
    const vW1 = Array(hiddenSize).fill(null).map(() => new Float64Array(inputSize));
    const mB1 = new Float64Array(hiddenSize);
    const vB1 = new Float64Array(hiddenSize);

    const mW2 = Array(outputSize).fill(null).map(() => new Float64Array(hiddenSize));
    const vW2 = Array(outputSize).fill(null).map(() => new Float64Array(hiddenSize));
    const mB2 = new Float64Array(outputSize);
    const vB2 = new Float64Array(outputSize);
    ...
    const z2 = new Float64Array(outputSize);
    const a2 = new Float64Array(outputSize);
    const dZ2 = new Float64Array(outputSize);
    const dZ1 = new Float64Array(hiddenSize);

    const numSamples = n - inputSize;
    
    // FIX DATA LEAKAGE: Chia tập Train/Validation (80/20) nghiêm ngặt
    const trainSize = Math.floor(numSamples * 0.8);
    const valSize = numSamples - trainSize;
    ...
    const epochs = Math.max(50, Math.min(baseEpochs, Math.round(maxTrainOps / Math.max(1, trainSize))));
    ...
    for (let ep = 0; ep < epochs; ep++) {
      ...
      // TRAINING PHASE (Chỉ huấn luyện trên 80% dữ liệu đầu)
      for (let i = 0; i < trainSize; i++) {
        ...
        // Sửa đúng chuẩn hàm mất mát Cross-Entropy & Softmax Derivative
        dZ2[0] = a2[0] - targetY0;
        dZ2[1] = a2[1] - targetY1;
        dZ2[2] = a2[2] - targetY2;
      ...
      // VALIDATION PHASE (Kiểm tra Overfitting trên 20% dữ liệu cuối, Early Stopping)
      let valLoss = 0;
      for (let i = trainSize; i < numSamples; i++) {
        ...
        for (let out = 0; out < outputSize; out++) {
          ...
          z2[out] = sumNode;
          if (sumNode > maxVal) maxVal = sumNode;
        }
        ...
        let predictedProb = 1e-7;
        if (targetType === 'TAI') predictedProb = a2[0] / sumExps;
        else if (targetType === 'XIU') predictedProb = a2[1] / sumExps;
        else if (targetType === 'HOA') predictedProb = a2[2] / sumExps;

        valLoss -= Math.log(predictedProb + 1e-7); // Cross-Entropy Loss
      }
      valLoss /= valSize;
    ...
    const pred_z1 = new Float64Array(hiddenSize);
    const pred_a1 = new Float64Array(hiddenSize);
    for (let h = 0; h < hiddenSize; h++) {
      let sumNode = B1[h];
      for (let inp = 0; inp < inputSize; inp++) {
    ...
    const pred_z2 = new Float64Array(outputSize);
    for (let out = 0; out < outputSize; out++) {
      let sumNode = B2[out];
      for (let h = 0; h < hiddenSize; h++) {
...
export const runLSTMForecast = (
  ...
  try {
    ...
    const normalizedSums = new Float64Array(rawChronological.length);
    for (let i = 0; i < rawChronological.length; i++) {
      const sum = rawChronological[i].numbers[0] + rawChronological[i].numbers[1] + rawChronological[i].numbers[2];
      normalizedSums[i] = (sum - 3) / 15;
    }

    let W_hh = new Float64Array(safeHiddenSize);
    let W_xh = new Float64Array(safeHiddenSize);
    let W_hy = new Float64Array(safeHiddenSize);
    let b_h = new Float64Array(safeHiddenSize);
    for (let i = 0; i < safeHiddenSize; i++) {
      W_hh[i] = (Math.random() - 0.5) * 0.1;
      W_xh[i] = (Math.random() - 0.5) * 0.1;
      W_hy[i] = (Math.random() - 0.5) * 0.1;
    }
    let b_y = 0;

    const lr = 0.05;

    const forward = (seq: Float64Array) => {
      let h = new Float64Array(safeHiddenSize);
    ...
    for (let epoch = 0; epoch < safeEpochs; epoch++) {
      ...
      for (let i = startIdx; i < normalizedSums.length - safeTimeSteps; i++) {
        const seq = normalizedSums.subarray(i, i + safeTimeSteps);
        const target = normalizedSums[i + safeTimeSteps];

        let h = new Float64Array(safeHiddenSize);
        let h_prev = new Float64Array(safeHiddenSize);
    ...
    const lastSeq = normalizedSums.subarray(normalizedSums.length - safeTimeSteps);
...
export const runXGBoostForecast = (
  ...
  try {
    ...
    const sums = new Float64Array(rawChronological.length);
    for (let i = 0; i < rawChronological.length; i++) {
      sums[i] = rawChronological[i].numbers[0] + rawChronological[i].numbers[1] + rawChronological[i].numbers[2];
    }
    
    // Build dataset
    const len = sums.length - safeLag;
    const X: Float64Array[] = new Array(len);
    const ySum = new Float64Array(len);

    for (let i = 0; i < len; i++) {
      const window = sums.subarray(i, i + safeLag);
      
      let ema = 0;
      let sumForMean = 0;
      for (let j = 0; j < safeLag; j++) {
        const val = window[j];
        ema += val * Math.pow(0.8, safeLag - j);
        sumForMean += val;
      }
      
      const momentum = window[safeLag - 1] - window[0];
      const mean = sumForMean / safeLag;
      let varianceSum = 0;
      for (let j = 0; j < safeLag; j++) {
        varianceSum += Math.pow(window[j] - mean, 2);
      }
      const stdDev = Math.sqrt(varianceSum / safeLag);
      
      const features = new Float64Array(safeLag + 3);
      features.set(window);
      features[safeLag] = ema;
      features[safeLag + 1] = momentum;
      features[safeLag + 2] = stdDev;
      
      X[i] = features;
      ySum[i] = sums[i + safeLag];
    }

    let ySumTotal = 0;
    for (let i = 0; i < len; i++) ySumTotal += ySum[i];
    const basePrediction = len > 0 ? ySumTotal / len : 0;
    
    let currentPredictions = new Float64Array(len);
    currentPredictions.fill(basePrediction);
    
    const trees: GBNode[] = [];

    // Huấn luyện chuỗi cây quyết định trên phần dư (Gradient Boosting residuals process)
    const residuals = new Float64Array(len);
    for (let t = 0; t < safeNTrees; t++) {
      for (let i = 0; i < len; i++) {
        residuals[i] = ySum[i] - currentPredictions[i];
      }
      // Pass Array.from(X) and Array.from(residuals) to buildGBTree to avoid type mismatch
      const tree = buildGBTree(Array.from(X).map(a => Array.from(a)), Array.from(residuals), 0, safeMaxDepth);
      trees.push(tree);

      for (let i = 0; i < len; i++) {
        currentPredictions[i] += safeLearningRate * predictGBTree(tree, Array.from(X[i]));
      }
    }

    // Dự đoán tương lai gần
    const window = sums.subarray(sums.length - safeLag);
    let ema = 0;
    let sumForMean = 0;
    for (let j = 0; j < safeLag; j++) {
      const val = window[j];
    ...
    const currentX = new Float64Array(safeLag + 3);
    currentX.set(window);
    currentX[safeLag] = ema;
    currentX[safeLag + 1] = momentum;
    currentX[safeLag + 2] = stdDev;
    
    let finalSumRaw = basePrediction;
    const currentXArray = Array.from(currentX);
    for (const tree of trees) {
      finalSumRaw += safeLearningRate * predictGBTree(tree, currentXArray);
    }
...
export const runTransformerForecast = (
  ...
  try {
    ...
    const len = rawChronological.length;
    ...
    const numSequences = len - safeSeqLen;
    const sequences: Float64Array[] = new Array(numSequences);
    const targets = new Float64Array(numSequences);
    
    for (let i = 0; i < numSequences; i++) {
      sequences[i] = sums.subarray(i, i + safeSeqLen);
      targets[i] = sums[i + safeSeqLen];
    }
    ...
    let sumExp = 0;
    const expDots = new Float64Array(numSequences);
    for (let s = 0; s < numSequences; s++) {
      const val = Math.exp(dotProducts[s] - maxDot); // Trừ tối đa để tránh tràn số thực (Numerical stability)
      expDots[s] = val;
      sumExp += val;
    }
    
    const attentionWeights = new Float64Array(numSequences);
    let predictedSumRaw = 0;
    for (let s = 0; s < numSequences; s++) {
      const weight = expDots[s] / (sumExp || 1);
      attentionWeights[s] = weight;
      predictedSumRaw += weight * targets[s];
    }
...
/**
 * TẠI SAO (Why): Thêm thuật toán DSP (Digital Signal Processing) cao cấp - Phân tích Phổ Fourier rời rạc (Discrete Fourier Transform - DFT).
 * Giúp trích xuất các thành phần sóng điều hòa trội nhất (Dominant Frequencies) nhằm phát hiện chu kỳ lặp lại (bệt/bẻ/xen kẽ) của cầu xúc xắc.
 * Tối ưu hóa tuyệt đối bằng Float64Array cho trình duyệt Safari iOS hoạt động cực kỳ mượt mà.
 */
export const runFourierSpectralForecast = (
  rawChronological: Draw[]
): {
  predictedSum: number;
  scores: { TAI: number; XIU: number; HOA: number };
  description: string;
} => {
  try {
    if (!rawChronological || rawChronological.length < 15) {
      return {
        predictedSum: 10,
        scores: { TAI: 34.0, XIU: 34.0, HOA: 32.0 },
        description: 'Chưa đủ khối lượng dữ liệu lịch sử để thực thi phân tích phổ Fourier rời rạc (Cần tối thiểu 15 kỳ).',
      };
    }

    // Tối ưu hóa Vectorization: Chuyển dữ liệu lịch sử sang mảng Float64 tĩnh
    const len = Math.min(128, rawChronological.length); // Giới hạn cửa sổ tín hiệu để phân tích cục bộ
    const signal = new Float64Array(len);
    let mean = 0;

    for (let i = 0; i < len; i++) {
      const idx = rawChronological.length - len + i;
      const sum = rawChronological[idx].numbers[0] + rawChronological[idx].numbers[1] + rawChronological[idx].numbers[2];
      signal[i] = isNaN(sum) ? 10.5 : sum;
      mean += signal[i];
    }
    mean /= len;

    // Lọc bỏ thành phần DC (Mean subtraction) để tín hiệu dao động quanh mức 0, tăng độ chính xác phân tích phổ
    for (let i = 0; i < len; i++) {
      signal[i] -= mean;
    }

    // Thực hiện DFT (Discrete Fourier Transform) cơ bản một cách tối ưu nhất (Vòng lặp tay siêu nhanh)
    // Phân tích các tần số từ k = 1 đến len/2 (Nyquist limit)
    const numFreqs = Math.floor(len / 2);
    const real = new Float64Array(numFreqs);
    const imag = new Float64Array(numFreqs);
    const magnitude = new Float64Array(numFreqs);

    let maxMag = -Infinity;
    let dominantK = 1; // Tần số chu kỳ trội nhất
    ...
    for (let k = 1; k < numFreqs; k++) {
      ...
      real[k] = r;
      imag[k] = im;
      magnitude[k] = Math.sqrt(r * r + im * im);

      // Tìm tần số có biên độ dao động (amplitude magnitude) lớn nhất
    ...
    // Ngoại suy chu kỳ (Extrapolation): Dự báo giá trị điểm tiếp theo n = len dựa trên hàm điều hòa trội nhất
    // Kết hợp biên độ và pha ban đầu của tần số trội nhất
    const amplitude = (2 * maxMag) / len;
    const phase = Math.atan2(imag[dominantK], real[dominantK]);
    const nextAngle = ((2 * Math.PI * dominantK) / len) * len; // Điểm tiếp theo
    
    // Dự báo biến động điều hòa có bù thêm mức Mean ban đầu
    let harmonicForecast = amplitude * Math.cos(nextAngle + phase) + mean;
    harmonicForecast = Math.max(3, Math.min(18, harmonicForecast));

    // Tính toán độ lệch Volatility thực tế để cân chỉnh độ tin cậy của Tài/Xỉu/Hòa
    let variance = 0;
    for (let i = 0; i < len; i++) {
    ...
    const finalSum = Math.round(harmonicForecast);

    // Tính toán xác suất dựa trên khoảng cách của giá trị ngoại suy tới trung tâm các cửa cược
    const t_dist = Math.abs(harmonicForecast - 14.5);
    const x_dist = Math.abs(harmonicForecast - 6.5);
    const h_dist = Math.abs(harmonicForecast - 10.5);

    // Co giãn phân phối qua Volatility (Biến động thấp -> tự tin bám sát chu kỳ điều hòa)
    const scaleFactor = Math.max(1, stdDev * 0.5);
    const rawTai = 1 / (Math.pow(t_dist, 2) / scaleFactor + 1);
    const rawXiu = 1 / (Math.pow(x_dist, 2) / scaleFactor + 1);
    const rawHoa = 1 / (Math.pow(h_dist, 2) / scaleFactor + 1.5);
    ...
    return {
      ...
      description: `Mô hình DSP Fourier Spectral (Phân tích Phổ rời rạc). Phát hiện tần số chu kỳ dao động trội nhất k=${dominantK} (Chu kỳ lặp T~${(len / dominantK).toFixed(1)} phiên) và áp dụng phép chiếu điều hòa để dự báo nhịp cầu xúc xắc.`
    };
  } catch (error) {
    console.error('Lỗi tính toán runFourierSpectralForecast:', error);
    return {
      predictedSum: 11,
      scores: { TAI: 34, XIU: 34, HOA: 32 },
      description: 'Lỗi bộ xử lý tín hiệu DSP Fourier. Đã tự động kích hoạt bộ dự phòng thống kê.'
    };
  }
};
