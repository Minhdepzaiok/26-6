...
export class BackgroundRetrainingQueue {
  ...
  private async autonomousSentinelCheck() {
    ...
    try {
      const rawDraws = localStorage.getItem('lottery_draws');
      if (!rawDraws) return;
      ...
      if (driftReport.isDriftDetected) {
        ...
        const existingJob = this.jobs.find(j => j.status === 'PENDING' || j.status === 'RUNNING');
        if (existingJob) return;

        // AI Sentinel tự quyết định xếp hàng huấn luyện
        console.warn(`[AI Sentinel] Cảnh báo độ lệch (PSI: ${driftReport.psiScore.toFixed(4)}). Tự động kích hoạt phản biện & huấn luyện...`);
        const jobId = this.addJob('mlp_auto', 'MLP Neural Network (Autonomous AI Sentinel)', 300, 'HIGH');
        
        const job = this.jobs.find(j => j.id === jobId);
        if (job) {
          job.logs.push(`[Hội Đồng Phản Biện AI] Kích hoạt phiên họp khẩn cấp. Phát hiện Data Drift (PSI=${driftReport.psiScore.toFixed(4)}).`);
          job.logs.push(`[Warren Buffett AI] Biên an toàn (Margin of Safety) bị xâm phạm nghiêm trọng! Đình chỉ dự đoán rủi ro, phân bổ tài nguyên cho huấn luyện lại.`);
          job.logs.push(`[Charlie Munger AI] Khoảng cách KS-test là ${driftReport.ksDistance.toFixed(4)}. Cấu trúc phân phối đã thay đổi. Phê duyệt hiệu chỉnh trọng số.`);
          job.logs.push(`[Đoạn Vĩnh Bình AI] Đã đến lúc thực hiện Bản Phận: Dừng lại và cập nhật hệ thống ngay lập tức.`);
          job.logs.push(`[Lý Lục AI] Hệ thống Vectorization 60FPS đã sẵn sàng. Khởi động tiến trình học thuật sâu...`);
        }
        ...
        EventBus.publish({
          ...
          sender: 'AI_SENTINEL',
          timestamp: Date.now(),
          payload: { message: `Phát hiện Concept Drift. Đã tự động lên lịch phản biện & học lại.`, severity: 'warning' }
        });
      }
    } catch (e) {
  ...
  private async executeJob(job: RetrainingJob): Promise<void> {
    ...
    try {
      ...
      job.logs.push(`[Hoàn tất] Mô hình đã được huấn luyện tối ưu! Đã lưu trữ trọng số và bộ tiền xử lý Pipeline (.pkl/.onnx simulation).`);
      
      // Update Pipeline and Serialize (Mô phỏng xuất file .pkl / .onnx)
        ...
        try {
          ...
          if (rawDraws) {
            ...
            if (Array.isArray(draws) && draws.length > 0) {
              const { DataPipeline } = await import('./pipeline');
              const newPipeline = new DataPipeline();
              // FIX DATA LEAKAGE: Chỉ fit pipeline trên 80% tập Train
              const trainLen = Math.floor(draws.length * 0.8);
              const trainData = draws.slice(0, trainLen);
              newPipeline.fit(trainData.length > 10 ? trainData : draws);
              localStorage.setItem('ai_pipeline_config', newPipeline.serialize());
